...
function Officers() {
  ...
  const posts = [
    ...
    { id: 2, name: 'Hazrat', role: 'Current Vice President', content: 'Hazrat is really good at cybersecurity and works in tech as technical support. He recently earned his CCNA.', linkedin: '', github: '' },
  ...
  const listItems = posts.map(a => <div key={a.id}><h1 className="is-size-2" key={a.id}>{a.name}</h1>
    ...
    <p key={a.id} className='is-size-4'><a href={a.linkedin}>{a.linkedin}</a></p>
    <p key={a.id} className='is-size-4'><a href={a.github}>{a.github}</a></p>
